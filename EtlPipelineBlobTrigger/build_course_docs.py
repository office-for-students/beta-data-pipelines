...
def build_link_item(key, lookup_table):
    item = {}
    keyw = key + 'W'
...
def build_links(locations, locids, raw_inst_data, raw_course_data):
    ...
    for item_detail in item_details:
        link_item = build_link_item(item_detail[0], item_detail[2])
        if link_item:
...
def build_course_entry(locations, locids, raw_inst_data, raw_course_data):
    ...
    distance_learning = build_code_label_entry(raw_course_data, lookup.distance_learning_lookup, 'DISTANCE')
    if distance_learning:
        course_entry['distance_learning'] = distance_learning
    foundataion_year = build_code_label_entry(raw_course_data, lookup.foundation_year_availability, 'FOUNDATION')
    if foundataion_year:
    ...
    length_of_course = build_code_label_entry(raw_course_data, lookup.length_of_course, 'NUMSTAGE')
    if length_of_course:
