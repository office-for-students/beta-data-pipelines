...
""" etl_pipeline: Execute the ETL pipeline based on a message queue trigger """
...
from etl_pipeline import course_docs
...
def etl_pipeline_main(
    ...
    try:
        ...
        logging.info(
            f"etl_pipeline message queue triggered\n"
        )

        function_start_datetime = datetime.today().strftime("%d-%m-%Y %H:%M:%S")

        logging.info(
            f"etl_pipeline function started on {function_start_datetime}"
        )
        ...
        message = f"etl_pipeline successfully finished on {function_end_datetime}"
        ...
        #     f"Automated data import failed on {function_fail_datetime} at etl_pipeline" + msgin.get_body().decode(
        #         "utf-8") + msgerror,
        #     f"Data Import {environment} - {function_fail_date} - Failed"
        # )

        message = f"etl_pipeline failed on {function_fail_datetime}"
