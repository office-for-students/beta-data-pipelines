...
class TestGetCollectionLink(unittest.TestCase):
    ...
    @mock.patch.dict('os.environ', values={'AzureCosmosDbDatabaseId': 'test-db-id', 'AzureCosmosDbUkRlpCollection': 'test-collection-id'}, clear=True)
    def test_get_collection_link(self):
        expected_link = 'dbs/' + self.db_id + '/colls/' + self.collection_id
        collection_link = get_collection_link(self.db_id_env, self.collection_id_env)
